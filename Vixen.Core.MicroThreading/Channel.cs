...
/// </summary>
/// <remarks>
///     <see cref="MicroThread" /> can send and receive to a <see cref="Channel" />. Depending on the
///     <see cref="Channel.Preference" />,
///     sending or receiving <see cref="MicroThread" /> might be suspended and yield execution to another
///     <see cref="MicroThread" />.
/// </remarks>
/// <typeparam name="T">The type of element handled by this channel.</typeparam>
// TODO: Thread-safety
public class Channel<T> {
    readonly Queue<ChannelMicroThreadAwaiter<T>> receivers = new();
    readonly Queue<ChannelMicroThreadAwaiter<T>> senders = new();

    /// <summary>
    ///     Gets or sets the preference, allowing you to customize how <see cref="Send" /> and <see cref="Receive" /> behave
    ...
    /// <summary>
    ///     Gets the balance, which is the number of <see cref="MicroThread" /> waiting to send (if greater than 0) or receive
    ///     (if smaller than 0).
    /// </summary>
    /// <value>
    ///     The balance.
    ...
    /// <summary>
    ///     Sends a value over the channel. If no other <see cref="MicroThread" /> is waiting for data, the sender will be
    ///     blocked.
    ...
    /// <summary>
    ///     Receives a value over the channel. If no other <see cref="MicroThread" /> is sending data, the receiver will be
    ///     blocked.
