...
public sealed class SerializerSettings {
    ...
    int preferredIndent;
    string specialCollectionMember;
    IObjectSerializerBackend objectSerializerBackend;
    IMemberNamingConvention _namingConvention;
    internal readonly Dictionary<Type, IYamlSerializable> serializers = new();
    ...
    public int PreferredIndent {
        ...
        set {
            if (value < 1) {
                throw new ArgumentOutOfRangeException(nameof(value), "Expecting value > 0");
            }
    ...
    /// <summary>
    ///     Gets or sets a value indicating whether the indentation is trying to less
    ...
    /// <summary>
    ///     Gets or sets a value indicating whether to enable sorting keys from dictionary to YAML mapping. Default is true.
    ///     See remarks.
    /// </summary>
    ...
    /// <summary>
    ///     Gets or sets a value indicating whether the property <see cref="List{T}.Capacity" /> should be emitted. Default is
    ///     false.
    /// </summary>
    ...
    /// <summary>
    ///     Gets or sets a value indicating whether to emit short type name (type, assembly name) or full
    ///     <see cref="Type.AssemblyQualifiedName" />. Default is false.
    /// </summary>
    /// <value><c>true</c> to emit short type name; otherwise, <c>false</c>.</value>
    public bool EmitShortTypeName {
        get => AssemblyRegistry.UseShortTypeName;
    ...
    public ITagTypeRegistry TagTypeRegistry => AssemblyRegistry;

    /// <summary>
    ///     Gets or sets the default <see cref="DataStyle" />. Default is <see cref="DataStyle.Normal" />. See
    ///     <see cref="DynamicStyleFormat" /> to understand the resolution of styles.
    /// </summary>
    ...
    ///     Expecting length >= 2 and at least a special character '.', '~', '-' (not
    ///     starting on first char for '-')
    /// </exception>
    /// <remarks>
    ///     A pure <see cref="System.Collections.IDictionary" /> or <see cref="System.Collections.ICollection" /> is a class
    ///     that inherits from these types but are not adding any
    ///     public properties or fields. When these types are pure, they are respectively serialized as a YAML mapping (for
    ///     dictionary) or a YAML sequence (for collections).
    ///     If the collection type to serialize is not pure, the type is serialized as a YAML mapping sequence that contains
    ///     the public properties/fields as well as a
    ///     special fielx (e.g. "~Items") that contains the actual items of the collection (either a mapping for dictionary or
    ///     a sequence for collections).
    ///     The <see cref="SpecialCollectionMember" /> is this special key that is used when serializing items of a non-pure
    ///     collection.
    /// </remarks>
    public string SpecialCollectionMember {
        get => specialCollectionMember;
        set {
            if (value == null) {
                throw new ArgumentNullException(nameof(value));
            }
    ...
    /// <param name="isAlias">
    ///     if set to <c>true</c> the new tag name is an alias to a type that has already a tag name
    ///     associated to it.
    /// </param>
