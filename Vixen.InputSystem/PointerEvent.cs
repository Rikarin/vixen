...
public class PointerEvent : InputEvent {
    ...
    /// <remarks>
    ///     The default mouse pointer will always be affected to the PointerId 0. On a tablet, a pen or each fingers will
    ...
    public override string ToString() =>
        $"Pointer {PointerId} {EventType}, {AbsolutePosition}, Delta: {AbsoluteDeltaPosition}, DT: {DeltaTime}, {nameof(IsDown)}: {IsDown}, {nameof(Pointer)}: {Pointer.Name}";

    /// <summary>
    ///     Clones the pointer event, this is useful if you intend to use it after this frame, since otherwise it would be
    ///     recycled by the input manager the next frame
    /// </summary>
