...
using Vixen.InputSystem.VirtualButtons;

namespace Vixen.InputSystem;

/// <summary>
///     Manages collecting input from connected input device in the form of <see cref="IInputDevice" /> objects. Also
///     provides some convenience functions for most commonly used devices
/// </summary>
public partial class InputManager {
    ...
    readonly List<IInputDevice> devices = new();

    readonly List<InputEvent> events = new();

    // readonly List<GestureEvent> currentGestureEvents = new();
    // readonly Dictionary<GestureConfig, GestureRecognizer> gestureConfigToRecognizer = new();
    readonly List<Dictionary<object, float>> virtualButtonValues = new();

    // Mapping of device guid to device
    readonly Dictionary<Guid, IInputDevice> devicesById = new();

    // List mapping GamePad index to the guid of the device
    readonly List<List<IGamePadDevice>> gamePadRequestedIndex = new();

    readonly List<IKeyboardDevice> keyboards = new();
    readonly List<IPointerDevice> pointers = new();
    readonly List<IGameControllerDevice> gameControllers = new();
    readonly List<IGamePadDevice> gamePads = new();
    readonly List<ISensorDevice> sensors = new();

    readonly Dictionary<Type, IInputEventRouter> eventRouters = new();

    IApplication application;

    readonly Dictionary<IInputSource, EventHandler<TrackingCollectionChangedEventArgs>>
        devicesCollectionChangedActions = new();
    ...
    /// <summary>
    ///     Gets the reference to the accelerometer sensor. The accelerometer measures all the acceleration forces applied on
    ///     the device.
    /// </summary>
    ...
    /// <summary>
    ///     Gets the reference to the user acceleration sensor. The user acceleration sensor measures the acceleration produce
    ///     by the user on the device (no gravity).
    /// </summary>
    ...
    public bool UseRawInput {
            ...
            set
            {
                ...
                InputSourceWindowsRawInput rawInputSource =
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           Sources.OfType<InputSourceWindowsRawInput>().FirstOrDefault();
    ...
    public static Vector2
        TransformPosition(SizeF fromSize, RectangleF destinationRectangle, Vector2 screenCoordinates) =>
        new(
    ...
    public void Initialize(IApplication application) {
        ...
        // this creates a better index assignment in the case where you have both an xbox controller and another controller at startup
    ...
    ///     If true will make sure that the mouse cursor position moves to the center of the client
    ///     window
    /// </param>
    ...
    /// <summary>
    ///     Unlock the mouse's position previously locked by calling <see cref="LockMousePosition" /> and restore the mouse
    ///     visibility.
    /// </summary>
    /// <remarks>This function has no effects on devices that does not have mouse</remarks>
    ...
    public IGamePadDevice? GetGamePadByIndex(int gamePadIndex) {
        ...
        return gamePadIndex >= gamePadRequestedIndex.Count
            ? null
            : gamePadRequestedIndex[gamePadIndex].FirstOrDefault();
    }
    ...
    public IEnumerable<IGamePadDevice>? GetGamePadsByIndex(int gamePadIndex) {
        ...
        return gamePadIndex >= gamePadRequestedIndex.Count ? null : gamePadRequestedIndex[gamePadIndex];
    }
    ...
    /// </summary>
    /// <remarks>
    ///     This method could take several milliseconds and should be used at specific time in a game where performance is not
    ///     crucial (pause, configuration screen...etc.)
    /// </remarks>
    public void Scan() {
    ...
    public void Update() {
        ...
        PreUpdateInput?.Invoke(this, new() { DeltaTime = Time.DeltaTime });

        // Send events to input listeners
        foreach (var evt in events) {
            if (!eventRouters.TryGetValue(evt.GetType(), out var router)) {
                throw new InvalidOperationException(
                    $"The event type {evt.GetType()} was not registered with the input manager and cannot be processed"
                );
            }
    ...
    ///     An index to a <see cref="VirtualButtonConfig" /> stored in the
    ///     <see cref="VirtualButtonConfigSet" />
    /// </param>
    ...
    public virtual float GetVirtualButton(int configIndex, object bindingName) {
        ...
        virtualButtonValues[configIndex].TryGetValue(bindingName, out var value);
        return value;
    }
    ...
    void SourcesOnCollectionChanged(object o, TrackingCollectionChangedEventArgs e) {
        ...
        switch (e.Action) {
            case NotifyCollectionChangedAction.Add:
                ...
                EventHandler<TrackingCollectionChangedEventArgs> eventHandler = (_, args) =>
                    InputDevicesOnCollectionChanged(source, args);
    ...
    public int GetFreeGamePadIndex(IGamePadDevice gamePad) {
        ...
        var targetIndex = 0;
        for (var i = 0; i < gamePadRequestedIndex.Count; i++) {
    ...
    void AddSources() {
        ...
        // var windowInputSource = InputSourceFactory.NewWindowInputSource(application);
...
// #endif
    ...
    // TODO
    // void GesturesOnCollectionChanged(
    //     object sender,
    ...
    // TODO
    // void StartGestureRecognition(GestureConfig config) {
    //     var aspectRatio = Pointer?.SurfaceAspectRatio ?? 1.0f;
    //     gestureConfigToRecognizer.Add(config, config.CreateRecognizer(aspectRatio));
    // }
    //
    // void StopGestureRecognition(GestureConfig config) {
    ...
    void InputDevicesOnCollectionChanged(IInputSource source, TrackingCollectionChangedEventArgs e) {
        switch (e.Action) {
            case NotifyCollectionChangedAction.Add:
    ...
    void OnInputDeviceAdded(IInputSource source, IInputDevice device) {
        devices.Add(device);
        if (devicesById.TryGetValue(device.Id, out var value)) {
            throw new InvalidOperationException(
                $"Device with Id {device.Id}({device.Name}) already registered to {value.Name}"
            );
        }

        devicesById.Add(device.Id, device);

        if (device is IKeyboardDevice keyboardDevice) {
            RegisterKeyboard(keyboardDevice);
            keyboards.Sort((l, r) => -l.Priority.CompareTo(r.Priority));
        } else if (device is IPointerDevice pointerDevice) {
            RegisterPointer(pointerDevice);
            pointers.Sort((l, r) => -l.Priority.CompareTo(r.Priority));
        } else if (device is IGameControllerDevice controllerDevice) {
            RegisterGameController(controllerDevice);
            gameControllers.Sort((l, r) => -l.Priority.CompareTo(r.Priority));
        } else if (device is IGamePadDevice padDevice) {
            RegisterGamePad(padDevice);
            gamePads.Sort((l, r) => -l.Priority.CompareTo(r.Priority));
        } else if (device is ISensorDevice sensorDevice) {
            RegisterSensor(sensorDevice);
        }
        ...
        DeviceAdded?.Invoke(
            this,
            new() { Device = device, Source = source, Type = DeviceChangedEventType.Added }
        );
    }

    void OnInputDeviceRemoved(IInputDevice device) {
        ...
        if (device is IKeyboardDevice keyboardDevice) {
            UnregisterKeyboard(keyboardDevice);
        } else if (device is IPointerDevice pointerDevice) {
            UnregisterPointer(pointerDevice);
        } else if (device is IGameControllerDevice controllerDevice) {
            UnregisterGameController(controllerDevice);
        } else if (device is IGamePadDevice padDevice) {
            UnregisterGamePad(padDevice);
        } else if (device is ISensorDevice sensorDevice) {
            UnregisterSensor(sensorDevice);
        }
        ...
        DeviceRemoved?.Invoke(
            this,
            new() { Device = device, Source = source, Type = DeviceChangedEventType.Removed }
        );
    }
    ...
    void RegisterGameController(IGameControllerDevice gameController) {
        gameControllers.Add(gameController);
    }

    void UnregisterGameController(IGameControllerDevice gameController) {
        gameControllers.Remove(gameController);
    }

    void GamePadOnIndexChanged(object sender, GamePadIndexChangedEventArgs e) {
        UpdateGamePadRequestedIndices();
    }

    void RegisterSensor(ISensorDevice sensorDevice) {
        sensors.Add(sensorDevice);
    }

    void UnregisterSensor(ISensorDevice sensorDevice) {
    ...
    /// </summary>
    void UpdateGamePadRequestedIndices() {
    ...
    // TODO
    // void UpdateGestureEvents(TimeSpan elapsedGameTime) {
    ...
    void UpdateVirtualButtonValues() {
        if (VirtualButtonConfigSet != null) {
            for (var i = 0; i < VirtualButtonConfigSet.Count; i++) {
    ...
    class InputEventRouter<TEventType> : IInputEventRouter where TEventType : InputEvent, new() {
        public HashSet<IInputEventListener> Listeners { get; } =
            new(ReferenceEqualityComparer<IInputEventListener>.Default);
