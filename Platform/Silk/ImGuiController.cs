...
public class ImGuiController : IDisposable {
    ...
    int _windowWidth;
    int _windowHeight;
    int _swapChainImageCt;
    Sampler _fontSampler;
    DescriptorSetLayout _descriptorSetLayout;
    DescriptorSet _descriptorSet;
    PipelineLayout _pipelineLayout;
    ShaderModule _shaderModuleVert;
    ShaderModule _shaderModuleFrag;
    Pipeline _pipeline;
    WindowRenderBuffers _mainWindowRenderBuffers;
    GlobalMemory _frameRenderBuffers;
    DeviceMemory _fontMemory;
    Image _fontImage;
    ImageView _fontView;
    ulong _bufferMemoryAlignment = 256;
    ...
    public void Render(CommandBuffer commandBuffer, Framebuffer framebuffer, Extent2D swapChainExtent) {
        if (_frameBegun) {
            _frameBegun = false;
            ImGui.Render();
            RenderImDrawData(ImGui.GetDrawData(), commandBuffer, framebuffer, swapChainExtent);
        }
    }
    ...
    unsafe void Init(
        ...
        if (swapChainImageCt < 2) {
            throw new("Swap chain image count must be >= 2");
        }
        ...
        _vk.CreateDescriptorPool(_device, descriptorPool, default, out _descriptorPool).EnsureSuccess();
        ...
        _vk.CreateRenderPass(_device, renderPassInfo, default, out _renderPass).EnsureSuccess();
        ...
        vk.CreateSampler(_device, info, default, out _fontSampler).EnsureSuccess();
        ...
        vk.CreateDescriptorSetLayout(_device, descriptorInfo, default, out _descriptorSetLayout).EnsureSuccess();
        ...
        fixed (DescriptorSetLayout* pg_DescriptorSetLayout = &_descriptorSetLayout) {
            ...
            vk.AllocateDescriptorSets(_device, alloc_info, out _descriptorSet).EnsureSuccess();
        }

        var vertPushConst = new PushConstantRange {
            StageFlags = ShaderStageFlags.VertexBit, Offset = sizeof(float) * 0, Size = sizeof(float) * 4
        };
        ...
        if (vk.CreatePipelineLayout(_device, layout_info, default, out _pipelineLayout) != Result.Success) {
            throw new("Unable to create the descriptor set layout");
        }
        ...
        var binding_desc = new VertexInputBindingDescription {
            Stride = (uint)Unsafe.SizeOf<ImDrawVert>(), InputRate = VertexInputRate.Vertex
        };
        ...
        var depth_info =
            new PipelineDepthStencilStateCreateInfo { SType = StructureType.PipelineDepthStencilStateCreateInfo };
        ...
        if (vk.CreateGraphicsPipelines(_device, default, 1, pipelineInfo, default, out _pipeline) != Result.Success) {
            throw new("Unable to create the pipeline");
        }
        ...
        var poolInfo = new CommandPoolCreateInfo {
            SType = StructureType.CommandPoolCreateInfo, QueueFamilyIndex = graphicsFamilyIndex
        };
        if (_vk.CreateCommandPool(_device, poolInfo, null, out var commandPool) != Result.Success) {
            throw new("failed to create command pool!");
        }
        ...
        if (_vk.AllocateCommandBuffers(_device, allocInfo, out var commandBuffer) != Result.Success) {
            throw new("Unable to allocate command buffers");
        }
        ...
        if (_vk.CreateImage(_device, imageInfo, default, out _fontImage) != Result.Success) {
            throw new("Failed to create font image");
        }
    ...
    uint GetMemoryTypeIndex(Vk vk, MemoryPropertyFlags properties, uint type_bits) {
    ...
    void OnKeyChar(IKeyboard arg1, char arg2) {
        _pressedChars.Add(arg2);
    }

    void WindowResized(Vector2D<int> size) {
        _windowWidth = size.X;
    ...
    void UpdateImGuiInput() {
        ...
        io.MousePos = new(point.X, point.Y);
    ...
    unsafe void RenderImDrawData(
        ...
        if (drawData.TotalVtxCount > 0) {
            ...
            if (_vk.MapMemory(
                    _device,
                    frameRenderBuffer.VertexBufferMemory,
                    0,
                ...
                != Result.Success) {
                throw new("Unable to map device memory");
            }
            ...
            if (_vk.FlushMappedMemoryRanges(_device, 2, range) != Result.Success) {
                throw new("Unable to flush memory to device");
            }
    ...
    unsafe void CreateOrResizeBuffer(
        ref Buffer buffer,
        ref DeviceMemory buffer_memory,
        ...
        if (_vk.BindBufferMemory(_device, buffer, buffer_memory, 0) != Result.Success) {
            throw new("Unable to bind device memory");
        }
