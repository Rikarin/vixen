...
public abstract class Store<T> : IDisposable where T : new() {
    ...
    static readonly bool LockEnabled = Platform.Type != PlatformType.MacOS;

    /// <summary>
    ///     Gets or sets a flag specifying if the index map changes should be kept aside instead of being committed
    ///     immediately.
    /// </summary>
    ...
    ///     Waits for pending operation to finish, if any. Note that it does not write pending transaction if
    ///     <see cref="Save" /> has not been called before.
    /// </summary>
    public void Dispose() {
        if (stream != null) {
            lock (stream) {
    ...
    public bool LoadNewValues() {
        ...
        lock (stream) {
            ...
            if (LockEnabled && stream is FileStream fileStream) {
                NativeLockFile.LockFile(fileStream, position, long.MaxValue - position, false);
            }
            ...
            try {
                ...
                if (LockEnabled && stream is FileStream stream1) {
                    NativeLockFile.UnlockFile(stream1, position, long.MaxValue - position);
                }
            }
    ...
    void SaveValues(IEnumerable<T> values, int currentTransaction) {
        if (stream == null) {
            throw new InvalidOperationException("No active stream.");
        }
        ...
        lock (stream) {
            ...
            if (LockEnabled && stream is FileStream fileStream) {
                NativeLockFile.LockFile(
                    fileStream,
                    indexStreamPosition,
                    long.MaxValue - indexStreamPosition,
                    true
                );
            }
    ...
    void SaveValue(T item, int currentTransaction) {
        if (stream == null) {
            throw new InvalidOperationException("No active stream.");
        }
    ...
    void RefreshData(long fileSize) {
    ...
    protected virtual object BuildContext(Stream stream) => stream;
