...
public class ObjectDatabase : IDisposable {
    ...
    static readonly Dictionary<ObjectId, Blob> LoadedBlobs = new();

    // When reading, first try backendRead2, then backendRead1.
    // When writing, try backendWrite.
    readonly IOdbBackend backendRead1;
    readonly IOdbBackend backendRead2;
    readonly IOdbBackend backendWrite;
    ...
    /// <summary>
    ///     Creates a new instance of the <see cref="ObjectDatabase" /> class using default database path, index name, and
    ///     local database path, and loading default bundle.
    /// </summary>
    ...
    public void Dispose() {
        ...
        if (backendWrite != null
            && !ReferenceEquals(backendRead2, backendWrite)
            && !ReferenceEquals(backendRead2, backendRead1)) {
    ...
    public string CreateBundle(
        ...
        var packUrl =
            bundleBackend.BundleDirectory
            + bundleName
            + BundleOdbBackend.BundleExtension; // we don't want the pack to be compressed in the APK on android
    ...
    public Task LoadBundle(string bundleName) => BundleBackend.LoadBundle(bundleName, ContentIndexMap);
    ...
    public IEnumerable<ObjectId> EnumerateLooseObjects() => backendRead1.EnumerateObjects();
    ...
    public bool Exists(ObjectId objectId) =>
        (BundleBackend != null && BundleBackend.Exists(objectId))
    ...
    public unsafe ObjectId Write(IntPtr data, int size, bool forceWrite = false) {
        ...
        var ums = new UnmanagedMemoryStream((byte*)data, size, size, FileAccess.Write);
    ...
    public Stream OpenStream(
        ...
        using var streamRead = OpenStreamForRead(
            objectId,
    ...
    public OdbStreamWriter CreateStream() => backendWrite.CreateStream();

    /// <summary>
    ///     Creates a in-memory binary blob as a <see cref="Blob" /> that will also be stored using the active
    ///     <see cref="IOdbBackend" />.
    ...
    public unsafe Blob CreateBlob(IntPtr data, int size) {
        // Generate hash
        ObjectId objectId;
        var nativeMemoryStream = new UnmanagedMemoryStream((byte*)data, size, size, FileAccess.Write);
        ...
        lock (LoadedBlobs) {
            ...
            if (blob == null) {
                ...
                // Create blob
                blob = new(this, objectId, data, size);
                blob.AddReference();
    ...
    /// <returns>
    ///     The <see cref="Blob" /> matching this <see cref="ObjectId" /> with an incremented reference count if it
