...
/// <summary>
///     A multi threaded wrapper over a Stream, used by the VirtualFileSystem.
///     It also allows restricted access to sub parts of the Stream (useful for serialization and data streaming).
/// </summary>
public class VirtualFileStream : Stream {
    ...
    readonly bool disposeInternalStream;
    ...
    public override long Length {
        get {
            if (endPosition == -1) { // Use underlying stream if not a sub stream
    ...
    public override long Position {
        get => InternalStream.Position - startPosition;
    ...
    /// <param name="disposeInternalStream">
    ///     if set to <c>true</c> this instance has ownership of the internal stream and will
    ///     dispose it].
    /// </param>
    ...
    ///     Attempted to create a VirtualFileStream from a Stream which doesn't
    ///     support seeking.
    /// </exception>
    public VirtualFileStream(
        ...
        if (internalStream is VirtualFileStream vfs) {
            ...
            if ((endPosition < startPosition || endPosition > virtualFileStream.endPosition)
                && virtualFileStream.endPosition != -1) {
    ...
    ///     The <paramref name="offset" /> and <paramref name="origin" />
    ///     would seek to a position before the start of the stream or after the end.
    /// </exception>
    ...
    ///     is not large enough to write the specified <paramref name="buffer" />.
    /// </exception>
    ...
    /// <exception cref="IOException">
    ///     The remaining capacity in the stream
    ///     is not large enough to write the specified <paramref name="value" />.
    /// </exception>
