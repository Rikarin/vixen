...
public class CommandBuildStep : BuildStep {
    ...
    public override void Clean(IExecuteContext executeContext, BuilderContext builderContext, bool deleteOutput) {
        ...
        var commandResultsFileStream = executeContext.ResultMap.OpenStream(
            commandHash,
            VirtualFileMode.OpenOrCreate,
            VirtualFileAccess.ReadWrite,
            VirtualFileShare.ReadWrite
        );
        var commandResultEntries =
            new ListStore<CommandResultEntry>(commandResultsFileStream) { AutoLoadNewValues = false };
        commandResultEntries.LoadNewValues();
        commandResultsFileStream.Close();

        var matchingResult = FindMatchingResult(executeContext, commandResultEntries.GetValues());
        if (matchingResult != null) {
            if (deleteOutput) {
                foreach (var outputObject in matchingResult.OutputObjects) {
    ...
    public override async Task<ResultStatus> Execute(IExecuteContext executeContext, BuilderContext builderContext) {
        ...
        try {
            ...
            if (ShouldExecute(executeContext, commandResultEntries.GetValues(), commandHash, out matchingResult)) {
                var stepInProgress = executeContext.IsCommandCurrentlyRunning(commandHash);
    ...
    async Task<ResultStatus> StartCommand(
        ...
        using (commandResultEntries) {
            ...
            if (!(Command.ShouldSpawnNewProcess()
                    ...
                    && (status = await builderContext.TryExecuteRemote(
                        Command,
                        builderContext,
                        executeContext,
                        commandContext
                    ))
                    != ResultStatus.NotProcessed)) {
            ...
            if (status == ResultStatus.NotProcessed) {
                throw new InvalidDataException(
                    "The command " + Command + " returned ResultStatus.NotProcessed after completion."
                );
            }
    ...
    public delegate Task<ResultStatus> TryExecuteRemoteDelegate(
        Command command,
        BuilderContext builderContext,
        IExecuteContext executeContext,
        LocalCommandContext commandContext
    );

    internal bool ShouldExecute(
        IExecuteContext executeContext,
        CommandResultEntry[] previousResultCollection,
    ...
    internal CommandResultEntry FindMatchingResult(
        IPrepareContext prepareContext,
        CommandResultEntry[] commandResultCollection
    ) {
        ...
        foreach (var entry in commandResultCollection) {
            var entryMatch = true;
            ...
            if (entry.OutputObjects.Any(
                    outputObject => !VirtualFileSystem.FileExists(
                        FileOdbBackend.BuildUrl(VirtualFileSystem.ApplicationDatabasePath, outputObject.Value)
                    )
                )) {
                entryMatch = false;
            }
