...
public interface IVirtualFileProvider : IDisposable {
    ...
    /// <summary>
    ///     Gets the absolute path and location if the specified path physically exist on the disk in an uncompressed form
    ///     (could be inside another file).
    /// </summary>
    ...
    /// <returns>
    ///     True if success, false if not supported and entry is found (note: even when true, the file might not actually
    ///     exists).
    /// </returns>
    ...
    ///     One of the enumeration values that specifies whether the search operation should include all subdirectories or only
    ...
    /// <summary>
    ///     Move the specified file specified from its sourceUrl to the location pointed by destinationUrl. Do not overwrite,
    ///     throw IOException if the file can't be moved.
    /// </summary>
    ...
    /// <summary>
    ///     Move the specified file specified from its sourceUrl to the location pointed by destinationUrl in the destination
    ///     provider. Do not overwrite, throw IOException if the file can't be moved.
    /// </summary>
