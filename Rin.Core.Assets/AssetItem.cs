...
public sealed class AssetItem : IFileSynchronizable {
    ...
    /// <summary>
    ///     Gets the directory where the assets will be stored on the disk relative to the <see cref="Package" />. The
    ///     directory
    ...
    public UFile FullPath {
        get {
            ...
            var locationAndExtension =
                AlternativePath ?? new UFile(Location + AssetRegistry.GetDefaultExtension(Asset.GetType()));
    ...
    /// <remarks>
    ///     By default, contains the last modified time of the asset from the disk. If IsDirty is also updated from false to
    ///     true
    ...
    ///     The new location that will be used in the cloned <see cref="AssetItem" />. If this parameter
    ///     is null, it keeps the original location of the asset.
    /// </param>
    /// <param name="newAsset">
    ///     The new asset that will be used in the cloned <see cref="AssetItem" />. If this parameter
    ///     is null, it clones the original asset. otherwise, the specified asset is used as-is in the new
    ///     <see cref="AssetItem" />
    ///     (no clone on newAsset is performed)
    /// </param>
    ...
    public AssetItem Clone(
        ...
        Asset? newAsset = null,
        AssetClonerFlags flags = AssetClonerFlags.None
    ) =>
        Clone(false, newLocation, newAsset, flags);

    /// <summary>
    ///     Clones this instance without the attached package.
    /// </summary>
    /// <param name="keepPackage">
    ///     if set to <c>true</c> copy package information, only used by the
    ///     <see cref="Analysis.AssetDependencyManager" />.
    /// </param>
    /// <param name="newLocation">
    ///     The new location that will be used in the cloned <see cref="AssetItem" />. If this parameter
    ///     is null, it keeps the original location of the asset.
    ...
    public AssetItem Clone(
        ...
        var item = new AssetItem(
            ...
            newAsset ?? AssetCloner.Clone(Asset, flags),
            keepPackage ? Package : null
        ) { isDirty = isDirty, SourceFolder = SourceFolder, version = Version, AlternativePath = AlternativePath };
    ...
    class AssetItemComparerById : IEqualityComparer<AssetItem> {
