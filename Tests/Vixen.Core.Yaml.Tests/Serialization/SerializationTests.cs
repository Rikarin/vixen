...
public class SerializationTests : YamlTest {
    ...
    public void CircularReference() {
        ...
        var buffer = new StringWriter();
        var original = new Y();
        original.Child = new() { Child = original, Child2 = original };
    ...
    public void DeserializeExplicitType() {
        ...
        var serializer = new Serializer(settings);
        var result = serializer.Deserialize(YamlFile("explicitType.yaml"), typeof(object));

        Assert.True(result is Z);
    ...
    public void DeserializeDictionary() {
        ...
        Assert.True(
            result is IDictionary<object, object>,
            "The deserialized object has the wrong type."
        );
    ...
    public void DeserializeExplicitDictionary() {
        ...
        var serializer = new Serializer(settings);
        var result = serializer.Deserialize(YamlFile("dictionaryExplicit.yaml"));

        Assert.True(
            result is IDictionary<string, int>,
            "The deserialized object has the wrong type."
        );
    ...
    public void DeserializeListOfDictionaries() {
        ...
        var result = serializer.Deserialize(
            YamlFile("listOfDictionaries.yaml"),
    ...
    public void SerializationOfNullWorksInJson() {
        var settings = new SerializerSettings { EmitDefaultValues = true, EmitJsonCompatible = true };
        settings.RegisterAssembly(typeof(X).Assembly);
        var serializer = new Serializer(settings);
    ...
    public void NullValuesInListsAreAlwaysEmittedWithoutEmitDefaults() {
        ...
        var serializer = new Serializer(new() { LimitPrimitiveFlowSequence = 0 });
        var writer = new StringWriter();
    ...
    public void NullValuesInListsAreAlwaysEmittedWithEmitDefaults() {
        var input = new[] { "foo", null, "bar" };
        var serializer = new Serializer(new() { EmitDefaultValues = true, LimitPrimitiveFlowSequence = 0 });
        var writer = new StringWriter();
...
Name: Andy
...";
        andy = serializer.DeserializeInto(yaml, andy);
        Assert.Equal("Andy", andy.Name);
        Assert.Equal(30, andy.Age);

        andy = new() { Name = "Not Andy", Age = 30 };
...
...";
    ...
    public void SerializeGenericDictionaryShouldNotThrowTargetException() {
        ...
        var buffer = new StringWriter();
        serializer.Serialize(buffer, new OnlyGenericDictionary { { "hello", "world" } });
    }
...
";
    ...
    T SerializeThenDeserialize<T>(T input) {
        var serializer = new Serializer();
        var writer = new StringWriter();
    ...
    public class CustomTypeConverter : ScalarSerializerBase, IYamlSerializableFactory {
        public IYamlSerializable TryCreate(SerializerContext context, ITypeDescriptor typeDescriptor) =>
            typeDescriptor.Type == typeof(SomeCustomType) ? this : null;

        public override object ConvertFrom(ref ObjectContext context, Scalar fromScalar) =>
            new SomeCustomType(fromScalar.Value);

        public override string ConvertTo(ref ObjectContext objectContext) =>
            ((SomeCustomType)objectContext.Instance).Value;
    }
    ...
    class OnlyGenericDictionary : IDictionary<string, string> {
