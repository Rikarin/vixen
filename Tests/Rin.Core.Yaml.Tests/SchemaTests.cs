...
public class SchemaTests {
    ...
    public void TestExtendedSchema() {
        ...
        TryParse(
            schema,
            "2002-12-14 21:59:43.234",
            ExtendedSchema.TimestampShortTag,
            new DateTime(2002, 12, 14, 21, 59, 43, 234)
        );
    }

    void TestFailsafeSchemaCommon(IYamlSchema schema) {
    ...
    void TestJsonSchemaCommon(IYamlSchema schema) {
        Assert.Equal(
            SchemaBase.StrShortTag,
            schema.GetDefaultTag(new Scalar(null, null, "true", ScalarStyle.DoubleQuoted, false, false))
        );
    ...
    void TestCoreSchemaCommon(IYamlSchema schema) {
        TestJsonSchemaCommon(schema);
    ...
    void TryParse(IYamlSchema schema, string scalar, string expectedShortTag, object expectedValue) {
        string tag;
        object value;
