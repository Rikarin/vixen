...
namespace Vixen.Platform.Vulkan.Allocator.Metadata;

sealed class BlockMetadata_Generic : IBlockMetadata {
    int freeCount;

    readonly LinkedList<Suballocation> suballocations = new();

    readonly List<LinkedListNode<Suballocation>> freeSuballocationsBySize = new();
    ...
    public void Alloc(
        ...
        if (paddingEnd > 0) {
            var newNode = suballocations.AddAfter(
                requestNode,
        ...
        if (paddingBegin > 0) {
            var newNode = suballocations.AddBefore(
                requestNode,
        ...
        if (freeSuballocCount > 0) {
            if (context.Strategy == AllocationStrategyFlags.BestFit) {
                var index = freeSuballocationsBySize.FindIndex(
                    context.AllocationSize + 2 * Helpers.DebugMargin,
    ...
    [Conditional("DEBUG")]
    void ValidateFreeSuballocationList() {
        long lastSize = 0;
    ...
    bool CheckAllocation(
        ...
        if (context.CanMakeOtherLost) {
            ...
            AccountForBackwardGranularityConflict(
                node,
    ...
    void MergeFreeWithNext(LinkedListNode<Suballocation> node) {
    ...
    LinkedListNode<Suballocation> FreeSuballocation(LinkedListNode<Suballocation> item) {
    ...
    void RegisterFreeSuballocation(LinkedListNode<Suballocation> item) {
    ...
    void UnregisterFreeSuballocation(LinkedListNode<Suballocation> item) {
        Debug.Assert(item.ValueRef.Type == SuballocationType.Free);
        Debug.Assert(item.ValueRef.Size > 0);
