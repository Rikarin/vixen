...
using Vixen.Platform.Vulkan.Allocator.Defragmentation;
...
public sealed unsafe class VulkanMemoryAllocator : IDisposable {
    ...
    readonly long PreferredLargeHeapBlockSize;
    readonly PhysicalDevice PhysicalDevice;
    int CurrentFrame;
    uint GPUDefragmentationMemoryTypeBits = uint.MaxValue;

    readonly ReaderWriterLockSlim PoolsMutex = new();
    readonly List<VulkanMemoryPool> Pools = new();
    ...
    ref PhysicalDeviceMemoryProperties.MemoryTypesBuffer MemoryTypes => ref memoryProperties.MemoryTypes;

    ref PhysicalDeviceMemoryProperties.MemoryHeapsBuffer MemoryHeaps => ref memoryProperties.MemoryHeaps;
    ...
    public int? FindMemoryTypeIndex(uint memoryTypeBits, in AllocationCreateInfo allocInfo) {
        ...
        if (((allocInfo.RequiredFlags | allocInfo.PreferredFlags)
                & (MemoryPropertyFlags.MemoryPropertyDeviceCoherentBitAmd
                    | MemoryPropertyFlags.MemoryPropertyDeviceUncachedBitAmd))
            == 0) {
    ...
    long CalcPreferredBlockSize(int memTypeIndex) {
    ...
    Allocation AllocateMemoryOfType(
        long size,
        long alignment,
        ...
        if ((finalCreateInfo.Flags & AllocationCreateFlags.Mapped) != 0
            && (MemoryTypes[memoryTypeIndex].PropertyFlags & MemoryPropertyFlags.MemoryPropertyHostVisibleBit) == 0) {
    ...
    Allocation AllocateDedicatedMemoryPage(
        long size,
        SuballocationType suballocType,
        int memTypeIndex,
        ...
        var allocation =
            new DedicatedAllocation(this, memTypeIndex, memory, suballocType, mappedData, size) { UserData = userData };
    ...
    Allocation AllocateDedicatedMemory(
        long size,
        SuballocationType suballocType,
        int memTypeIndex,
        bool withinBudget,
        ...
        var allocInfo = new MemoryAllocateInfo {
            SType = StructureType.MemoryAllocateInfo, MemoryTypeIndex = (uint)memTypeIndex, AllocationSize = (ulong)size
        };
        ...
        if (UseKhrBufferDeviceAddress) {
            ...
            if (dedicatedInfo.DedicatedBuffer.Handle != default) {
                canContainBufferWithDeviceAddress = dedicatedInfo.DedicatedBufferUsage == UnknownBufferUsage
                    || (dedicatedInfo.DedicatedBufferUsage & BufferUsageFlags.BufferUsageShaderDeviceAddressBitKhr)
                    != 0;
            } else if (dedicatedInfo.DedicatedImage.Handle != default) {
    ...
    void FreeDedicatedMemory(DedicatedAllocation allocation) {
    ...
    uint CalculateGpuDefragmentationMemoryTypeBits() => throw new NotImplementedException();

    uint CalculateGlobalMemoryTypeBits() {
    ...
    internal long GetMemoryTypeMinAlignment(int memTypeIndex) =>
        IsMemoryTypeNonCoherent(memTypeIndex)
    ...
    internal void GetBufferMemoryRequirements(
        ...
        var req = new BufferMemoryRequirementsInfo2 {
            SType = StructureType.BufferMemoryRequirementsInfo2, Buffer = buffer
        };

        var dedicatedRequirements =
            new MemoryDedicatedRequirements { SType = StructureType.MemoryDedicatedRequirements };

        var memReq2 =
            new MemoryRequirements2 { SType = StructureType.MemoryRequirements2, PNext = &dedicatedRequirements };
        ...
        if (UseExtMemoryBudget) {
            ...
            try {
                ...
                if (heapBudget.VulkanUsage + outBudget.BlockBytes > heapBudget.BlockBytesAtBudgetFetch) {
                    outBudget.Usage = heapBudget.VulkanUsage
    ...
    internal Result
        DefragmentationPassBegin(ref DefragmentationPassMoveInfo[] passInfo, DefragmentationContext context) =>
        throw new NotImplementedException();
    ...
    internal int MakePoolAllocationsLost(VulkanMemoryPool pool) => pool.BlockList.MakePoolAllocationsLost(CurrentFrame);
    ...
    internal void FillAllocation(Allocation allocation, byte pattern) {
        if (Helpers.DebugInitializeAllocations
        ...
        if (size > 0 && IsMemoryTypeNonCoherent(memTypeIndex)) {
            ...
            if (allocation is BlockAllocation blockAlloc) {
                ...
                if (size == long.MaxValue) {
                    ...
                    memRange.Size = (ulong)Helpers.AlignUp(
