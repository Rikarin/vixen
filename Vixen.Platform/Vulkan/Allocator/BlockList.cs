...
class BlockList : IDisposable {
    ...
    readonly List<VulkanMemoryBlock> blocks = new();
    readonly ReaderWriterLockSlim mutex = new(LockRecursionPolicy.NoRecursion);

    readonly int minBlockCount, maxBlockCount;
    readonly bool explicitBlockSize;

    readonly Func<long, IBlockMetadata> metaObjectCreate;
    ...
    IEnumerable<VulkanMemoryBlock> BlocksInReverse //Just gonna take advantage of C#...
    {
        get {
        ...
        if (canMakeOtherLost) {
            ...
            for (; tryIndex < AllocationTryCount; ++tryIndex) {
                ...
                if (bestRequestBlock != null) {
                    ...
                    if (bestRequestBlock.MetaData.MakeRequestedAllocationsLost(
                            currentFrame,
                            FrameInUseCount,
    ...
    unsafe Result CreateBlock(long blockSize, out int newBlockIndex) {
        newBlockIndex = -1;
    ...
    void FreeEmptyBlocks(ref DefragmentationStats stats) {
        for (var i = blocks.Count - 1; i >= 0; --i) {
            var block = blocks[i];
    ...
    void Remove(VulkanMemoryBlock block) {
