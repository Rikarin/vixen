...
using PropertyDescriptor = Vixen.Core.Reflection.MemberDescriptors.PropertyDescriptor;
...
public class ObjectDescriptor : ITypeDescriptor {
    static readonly List<IMemberDescriptor> EmptyMembers = [];

    readonly ITypeDescriptorFactory factory;
    IMemberDescriptor[] members;
    Dictionary<string, IMemberDescriptor> mapMembers;
    HashSet<string> remapMembers;
    ...
    public bool IsMemberRemapped(string name) => remapMembers != null && remapMembers.Contains(name);
    ...
    public virtual void Initialize(IComparer<object> keyComparer) {
        ...
        mapMembers = new(members.Length);

        foreach (var member in members) {
            if (mapMembers.TryGetValue(member.Name, out var existingMember)) {
                throw new InvalidOperationException(
                    $"Failed to get ObjectDescriptor for type [{Type.FullName}]. The member [{member}] cannot be registered as a member with the same name is already registered [{existingMember}]"
                );
            }
            ...
            if (member.AlternativeNames != null) {
                foreach (var alternateName in member.AlternativeNames) {
                    if (mapMembers.TryGetValue(alternateName, out existingMember)) {
                        throw new InvalidOperationException(
                            $"Failed to get ObjectDescriptor for type [{Type.FullName}]. The member [{member}] cannot be registered as a member with the same name [{alternateName}] is already registered [{existingMember}]"
                        );
                    }
    ...
    protected virtual List<IMemberDescriptor> PrepareMembers() {
        ...
        foreach (var metadataTypeAttr in metadataTypeAttributes) {
            ...
            metadataClassMemberInfos.AddRange(
                from propertyInfo in metadataType.GetProperties(bindingFlags)
            ...
            metadataClassMemberInfos.AddRange(
                from fieldInfo in metadataType.GetFields(bindingFlags)
        ...
        var memberList = (from propertyInfo in Type.GetProperties(bindingFlags)
            ...
            select new PropertyDescriptor(
                ...
                propertyInfo,
                NamingConvention.Comparer
            )
            into member
            where PrepareMember(
                member,
                metadataClassMemberInfos
                    ?.FirstOrDefault(x => x.MemberInfo.Name == member.OriginalName && x.MemberType == member.Type)
                    .MemberInfo
            )
    ...
    protected virtual bool PrepareMember(MemberDescriptorBase member, MemberInfo metadataClassMemberInfo) {
        ...
        if (member.Mode == DataMemberMode.Default) {
            ...
            if (!member.HasSet
                && (memberType == typeof(string) || !memberType.IsClass)
        ...
        var shouldSerialize = Type.GetMethod(
            "ShouldSerialize" + member.OriginalName,
            ...
            member.ShouldSerialize = (obj, parentTypeMemberDesc) => {
                if (parentTypeMemberDesc?.HasDefaultValue ?? false) {
                    ...
                    if (parentDefaultValue != null) {
                        ...
                        var parentDefaultValueMemberValue =
                            member.Get(parentDefaultValue); // This is the real default value for this object
        ...
        member.Name = !string.IsNullOrEmpty(memberAttribute?.Name)
            ? memberAttribute.Name
            : NamingConvention.Convert(member.OriginalName);
    ...
    protected bool IsMemberToVisit(MemberInfo memberInfo) {
        ...
        if (memberInfo is PropertyInfo
            && memberInfo is { Name: "SyncRoot", DeclaringType: not null }
            && (memberInfo.DeclaringType.Namespace ?? string.Empty).StartsWith(
