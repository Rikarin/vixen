...
public class BundleOdbBackend : IOdbBackend {
    ...
    readonly Dictionary<ObjectId, ObjectLocation> objects = new();

    // Bundle name => Bundle VFS URL
    readonly Dictionary<string, string> resolvedBundles = new();

    readonly List<LoadedBundle> loadedBundles = new();

    readonly ObjectDatabaseContentIndexMap contentIndexMap = new ObjectDatabaseContentIndexMap();
    ...
    /// <param name="objectDatabaseContentIndexMap">
    ///     The object database asset index map, where newly loaded assets will be
    ///     merged (ignored if null).
    /// </param>
    ...
    public async Task LoadBundleFromUrl(
        string bundleName,
        ...
        lock (loadedBundles) {
            ...
            if (loadedBundle == null) {
                loadedBundle = new() {
                    ...
                    ReferenceCount = 1,
                    Files = files,
                    Streams = new(files.Select(x => (Stream)null))
                };
        ...
        lock (objects) {
            foreach (var objectEntry in bundle.Objects) {
                objects[objectEntry.Key] = new() { Info = objectEntry.Value, LoadedBundle = loadedBundle };
            }
        }
    ...
    public static BundleDescription ReadBundleHeader(string bundleUrl, out List<string> bundleUrls) {
        ...
        var currentBundleExtensionUrl = bundleUrl.Length
            - (bundleUrl.EndsWith(BundleExtension, StringComparison.OrdinalIgnoreCase) ? BundleExtension.Length : 0);
        ...
        bundleUrls = new() { bundleUrl };
        bundleUrls.AddRange(
            bundle.IncrementalBundles.Select(x => bundleUrl.Insert(currentBundleExtensionUrl, "." + x))
        );
    ...
    public void UnloadBundle(string bundleName, ObjectDatabaseContentIndexMap objectDatabaseContentIndexMap) {
        ...
        lock (objects) {
            ...
            foreach (var otherLoadedBundle in loadedBundles) {
                ...
                // Read objects
                foreach (var objectEntry in bundle.Objects) {
                    objects[objectEntry.Key] = new() { Info = objectEntry.Value, LoadedBundle = otherLoadedBundle };
                }
    ...
    public static void CreateBundle(
        ...
        IOdbBackend backend,
        ObjectId[] objectIds,
        ISet<ObjectId> disableCompressionIds,
        Dictionary<string, ObjectId> indexMap,
        IList<string> dependencies,
        bool useIncrementalBundle
    ) {
        ...
        for (var i = 0; i < objectIds.Length; ++i) {
            ...
            objects.Add(new(objectIds[i], new()));
        }

        var incrementalBundles = new List<ObjectId>();

        // If there is a .bundle, add incremental id before it
        var bundleExtensionLength = bundleUrl.EndsWith(BundleExtension, StringComparison.OrdinalIgnoreCase)
            ? BundleExtension.Length
            : 0;

        // Early exit if package didn't change (header-check only)
        if (VirtualFileSystem.FileExists(bundleUrl)) {
            try {
                using (var packStream = VirtualFileSystem.OpenStream(
                           bundleUrl,
                    ...
                    if (ArrayExtensions.ArraysEqual(bundle.Dependencies, dependencies)
                        && ArrayExtensions.ArraysEqual(
                            bundle.Assets.OrderBy(x => x.Key).ToList(),
                            indexMap.OrderBy(x => x.Key).ToList()
                        )
                        && ArrayExtensions.ArraysEqual(
                            bundle.Objects.Select(x => x.Key).OrderBy(x => x).ToList(),
                            objectIds.OrderBy(x => x).ToList()
                        )) {
                        ...
                        if (useIncrementalBundle == bundle.IncrementalBundles.Count > 0
                            && bundle.IncrementalBundles.Select(
                                    x => bundleUrl.Insert(bundleUrl.Length - bundleExtensionLength, "." + x)
                                )
                                .All(
                                    x => {
                                        ...
                                        using var incrementalStream = VirtualFileSystem.OpenStream(
                                            x,
                                            VirtualFileMode.Open,
                                            VirtualFileAccess.Read
                                        );
                ...
                var directory = VirtualFileSystem.GetParentDirectory(bundleUrl);

                foreach (var incrementalBundleUrl in VirtualFileSystem.ListFiles(
                                 directory,
                                 filename.Insert(filename.Length - bundleExtensionLength, ".*"),
                                 VirtualSearchOption.TopDirectoryOnly
                             )
                             .Result) {
                    var incrementalIdString = incrementalBundleUrl.Substring(
                        incrementalBundleUrl.Length - bundleExtensionLength - ObjectId.HashStringLength,
                    ...
                    try {
                        ...
                        var reuseRatio = (float)(sizeNeededItems / (double)sizeTotal);
                        if (reuseRatio < 0.5f) {
                            VirtualFileSystem.FileDelete(incrementalBundleUrl);
                        } else {
                            ...
                            foreach (var @object in incrementalBundle.Objects) {
                                if (objectsToIndex.TryGetValue(@object.Key, out var objectIndex)) {
                                    ...
                                    objects[objectIndex] = new(@object.Key, objectInfo);
                                }
                            }
        ...
        if (useIncrementalBundle) {
            for (var i = 0; i < objectIds.Length; ++i) {
                ...
                incrementalObjects.Add(new(objects[i].Key, new()));
            }
        }
        ...
        using (var packStream = VirtualFileSystem.OpenStream(
            ...
            using (var incrementalStream = incrementalObjects.Count > 0
                       ? VirtualFileSystem.OpenStream(
                           bundleUrl.Insert(bundleUrl.Length - bundleExtensionLength, "." + newIncrementalId),
                           VirtualFileMode.Create,
                           VirtualFileAccess.Write
                       )
                       : null) {
                var incrementalBinaryWriter =
                    incrementalStream != null ? new BinarySerializationWriter(incrementalStream) : null;
                ...
                for (var i = 0; i < objectIds.Length; ++i) {
                    ...
                    using (var objectStream = backend.OpenStream(objectIds[i])) {
                        ...
                        objectInfo.IncrementalBundleIndex =
                            objectOutputStream == incrementalStream ? incrementalBundleIndex + 1 : 0;
                        objects[i] = new(objectIds[i], objectInfo);

                        if (useIncrementalBundle) {
                            // Also update incremental bundle object info
                            objectInfo.IncrementalBundleIndex = 0; // stored in same bundle
                            incrementalObjects[incrementalObjectIndex++] = new(objectIds[i], objectInfo);
                        }
                    }
                }
    ...
    public Stream OpenStream(
        ...
        lock (streams) {
            ...
            if ((stream = streams[objectLocation.Info.IncrementalBundleIndex]) != null) {
                ...
                stream = VirtualFileSystem.OpenStream(
                    loadedBundle.Files[objectLocation.Info.IncrementalBundleIndex],
                    VirtualFileMode.Open,
                    VirtualFileAccess.Read
                );
            }
        ...
        if (objectLocation.Info.IsCompressed) {
            ...
            return new PackageFileStreamLZ4(
                this,
                objectLocation,
                stream,
                CompressionMode.Decompress,
                objectLocation.Info.SizeNotCompressed,
                objectLocation.Info.EndOffset - objectLocation.Info.StartOffset
            );
        }

        return new PackageFileStream(
            this,
            objectLocation,
            stream,
            objectLocation.Info.StartOffset,
            objectLocation.Info.EndOffset,
            false
        );
    }
    ...
    public OdbStreamWriter CreateStream() => throw new NotSupportedException();
    ...
    public string GetFilePath(ObjectId objectId) => throw new NotSupportedException();
    ...
    public bool TryGetObjectLocation(ObjectId objectId, out string filePath, out long start, out long end) {
        ...
        lock (objects) {
            ...
            if (!objects.TryGetValue(objectId, out var location)) {
                return false;
            }
            ...
            var result = providerResult.Provider.TryGetFileLocation(
                providerResult.Path,
                out filePath,
                out var bundleStart,
                out var bundleEnd
    ...
    public void DeleteBundles(Func<string, bool> bundleFileDeletePredicate) {
        var bundleFiles = VirtualFileSystem.ListFiles(
                BundleDirectory,
                "*" + BundleExtension,
                VirtualSearchOption.TopDirectoryOnly
            )
        ...
        var bundleFilesGroups = bundleFiles.GroupBy(
            bundleUrl => {
                // Remove incremental ID from bundle url
                ObjectId incrementalId;
                var filename = VirtualFileSystem.GetFileName(bundleUrl);
                var bundleExtensionLength = filename.EndsWith(BundleExtension, StringComparison.OrdinalIgnoreCase)
                    ? BundleExtension.Length
                    : 0;
                if (filename.Length - bundleExtensionLength >= ObjectId.HashStringLength + 1
                    && filename[filename.Length - bundleExtensionLength - ObjectId.HashStringLength - 1] == '.'
                    && ObjectId.TryParse(
                        filename.Substring(
                            filename.Length - bundleExtensionLength - ObjectId.HashStringLength,
                            ObjectId.HashStringLength
                        ),
                        out incrementalId
                    )) {
                    bundleUrl = bundleUrl.Remove(
                        bundleUrl.Length - bundleExtensionLength - ObjectId.HashStringLength - 1,
                        1 + ObjectId.HashStringLength
                    );
                }
    ...
    Task<string> DefaultBundleResolve(string bundleName) {
    ...
    async Task<string> ResolveBundle(string bundleName, bool throwExceptionIfNotFound) {
        string bundleUrl;
    ...
    void UnloadBundleRecursive(string bundleName, ObjectDatabaseContentIndexMap objectDatabaseContentIndexMap) {
        if (bundleName == null) {
            throw new ArgumentNullException(nameof(bundleName));
        }

        lock (loadedBundles) {
            var loadedBundleIndex = -1;

            for (var index = 0; index < loadedBundles.Count; index++) {
            ...
            if (--loadedBundle.ReferenceCount == 0) {
                ...
                lock (loadedBundle.Streams) {
                    for (var index = 0; index < loadedBundle.Streams.Count; index++) {
    ...
    static bool ValidateHeader(Stream stream) {
        var binaryReader = new BinarySerializationReader(stream);
    ...
    class PackageFileStreamLZ4 : LZ4Stream {
        readonly BundleOdbBackend bundleOdbBackend;
        readonly ObjectLocation objectLocation;
        readonly Stream innerStream;

        public PackageFileStreamLZ4(
            ...
            : base(
                innerStream,
                compressionMode,
                uncompressedSize: uncompressedStreamSize,
                compressedSize: compressedSize,
                disposeInnerStream: false
            ) {
    ...
    class PackageFileStream : VirtualFileStream {
        ...
        public PackageFileStream(
            BundleOdbBackend bundleOdbBackend,
            ObjectLocation objectLocation,
            Stream internalStream,
            long startPosition = 0,
            long endPosition = -1,
            bool disposeInternalStream = true,
            bool seekToBeginning = true
        )
