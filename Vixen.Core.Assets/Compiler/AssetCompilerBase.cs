using Vixen.Core.Assets.Analysis;
...
public abstract class AssetCompilerBase : IAssetCompiler {
    ...
    public AssetCompilerResult Prepare(AssetCompilerContext context, AssetItem assetItem) {
        ...
        var result = new AssetCompilerResult(GetType()) { BuildSteps = new AssetBuildStep(assetItem) };
    ...
    static bool EnsureSourcesExist(AssetCompilerResult result, AssetItem assetItem) {
        if (result == null) {
        ...
        foreach (var member in sourceMembers) {
            ...
            if (!File.Exists(assetSource)) {
                result.Log.Error($"Unable to find the source file '{assetSource}' for Asset [{assetItem}]");
                return false;
            }
        }
    ...
    ///     The <paramref name="relativePath" /> argument is a null or empty
    ///     <see cref="UFile" />.
    /// </exception>
