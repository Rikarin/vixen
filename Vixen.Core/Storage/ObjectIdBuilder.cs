...
public unsafe struct ObjectIdBuilder {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Write(byte[] buffer, int offset, int count) => Write(buffer.AsSpan(offset, count));
    ...
    /// <param name="buffer">The buffer.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Write<T>(ReadOnlySpan<T> buffer) where T : unmanaged => Write(MemoryMarshal.AsBytes(buffer));
    ...
    public void Write(ReadOnlySpan<byte> span) {
        ref var currentBlock = ref Unsafe.As<uint, byte>(ref currentBlock1);
        var position = Length % 16;
        ref var buffer = ref Unsafe.AsRef(in span.GetPinnableReference());
...
#warning PERF: Do not copy byte-for-byte.
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [Obsolete("Use BodyCore(ref byte)")]
    void BodyCore(byte* data) {
        var b = (uint*)data;
