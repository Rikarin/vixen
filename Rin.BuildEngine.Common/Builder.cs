...
public class Builder : IDisposable {
    ...
    public static void OpenObjectDatabase(string buildPath, string indexName) {
        ...
        if (ObjectDatabase == null) {
            ...
            ObjectDatabase = new(VirtualFileSystem.ApplicationDatabasePath, indexName, null, false);
        }
    }
    ...
    public BuildResultCode Run(Mode mode, bool writeIndexFile = true) {
        ...
        var inputHashes = FileVersionTracker.GetDefault();
        {
            ...
            var threads = Enumerable.Range(0, ThreadCount)
                .Select(x => new Thread(SafeAction.Wrap(RunUntilEnd)) { IsBackground = true })
                .ToArray();
    ...
    public void WriteIndexFile(bool mergeWithCurrentIndexFile) {
        ...
        using (var indexFile = ContentIndexMap.NewTool(indexName)) {
            ...
            foreach (var outputObject in Root.OutputObjects.Where(x => x.Key.Type == UrlType.Content)
                         .Select(x => x.Value)) {
    ...
    static IEnumerable<CommandBuildStep> CollectCommandSteps(BuildStep step) {
    ...
    static void CollectContentReferenceDependencies(BuildStep step, HashSet<string> locations) {
    ...
    /// <remarks>
    ///     Each BuildStep inheriting from <see cref="BuildStep.OutputLocation" /> is considered as a top-level dependency step
    ///     that can have depedencies
    ///     on other top-level dependency. We are collecting all of them here.
    /// </remarks>
    static void PrepareDependencyGraph(
        BuildStep step,
        Dictionary<string, KeyValuePair<BuildStep, HashSet<string>>> contentBuildSteps
    ) {
        ...
        if (outputLocation != null) {
            ...
            if (!contentBuildSteps.ContainsKey(outputLocation)) {
                contentBuildSteps.Add(outputLocation, new(step, dependencies));
    ...
    void ComputeDependencyGraph(Dictionary<string, KeyValuePair<BuildStep, HashSet<string>>> contentBuildSteps) {
        foreach (var item in contentBuildSteps) {
            ...
            foreach (var dependency in dependencies) {
                ...
                if (contentBuildSteps.TryGetValue(dependency, out deps)) {
                    ...
                    Logger.Error(
                        $"BuildStep [{step}] depends on [{dependency}] but nothing that generates it could be found (or maybe incremental dependencies need to be implemented)"
                    );
                }
            }
        }
    ...
    /// <summary>
    ///     Collects dependencies between <see cref="BuildStep.OutputLocation" /> and fill the
    ///     <see cref="BuildStep.PrerequisiteSteps" /> accordingly.
    /// </summary>
    ...
    void PreRun() {
        ...
        var versionFile = Path.Combine(
            VirtualFileSystem.GetAbsolutePath(VirtualFileSystem.ApplicationDatabasePath),
            "version"
        );
    ...
    void ScheduleBuildStep(
        BuilderContext builderContext,
        BuildStep instigator,
        BuildStep buildStep,
        ...
        if (buildStep.ExecutionId == 0) {
            ...
            var executeContext =
                new ExecuteContext(this, builderContext, buildStep, logger) { Variables = new(variables) };
                    ...
                    async () => {
                        ...
                        if (buildStep.ArePrerequisitesSuccessful) {
                            try {
                                ...
                                executeContext.Logger.Warning(
                                    "A child task of build step "
                                    + buildStep
                            ...
                            if (microThread.Exception != null) {
                                executeContext.Logger.Error(
                                    "Exception in command " + buildStep + ": " + microThread.Exception
                                );
                                status = ResultStatus.Failed;
                            }
                        } else {
    ...
    class ExecuteContext : IExecuteContext {
        ...
        public ExecuteContext(Builder builder, BuilderContext builderContext, BuildStep buildStep, ILogger logger) {
            Logger = logger;
            this.builderContext = builderContext;
            this.builder = builder;
            this.buildStep = buildStep;
            buildTransaction = new(null, buildStep.GetOutputObjectsGroups());
        }
