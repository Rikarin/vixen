...
namespace Vixen.Core.Design.IO;

/// <summary>
///     Base class that describes a uniform path and provides method to manipulate them. Concrete class are
///     <see cref="UFile" /> and <see cref="UDirectory" />.
///     This class is immutable and its descendants are immutable. See remarks.
/// </summary>
/// <remarks>
///     <para>
///         A uniform path contains only characters '/' to separate directories and doesn't contain any successive
///         '/' or './'. This class is used to represent a path, relative or absolute to a directory or filename.
///     </para>
...
public abstract class UPath : IEquatable<UPath>, IComparable {
    ...
    /// <summary>
    ///     Volume separator char ':' used to separate volume on Windows
    /// </summary>
    ...
    // TODO: check is this is correct on Linux/Mac
    static readonly HashSet<char> InvalidFileNameChars = new(
        new[] {
    ...
    public bool IsAbsolute =>
        HasDrive || (DirectorySpan.IsValid && FullPath[DirectorySpan.Start] == DirectorySeparatorChar);
    ...
    internal UPath(string? filePath, bool isDirectory) {
        if (!isDirectory
            && filePath != null
            && (
                filePath.EndsWith(DirectorySeparatorChar)
                || filePath.EndsWith(DirectorySeparatorCharAlt)
                || filePath.EndsWith(VolumeSeparatorChar)
            )
           ) {
    ...
    public static bool IsNullOrEmpty(UPath? path) => string.IsNullOrEmpty(path?.FullPath);

    /// <summary>
    ///     Gets the drive (contains the ':' if any), can be null.
    /// </summary>
    /// <returns>The drive.</returns>
    public string? GetDrive() => DriveSpan.IsValid ? FullPath.Substring(DriveSpan) : null;
        ...
        if (DirectorySpan.IsValid) {
            ...
            if (index >= 0) {
                ...
                return new(
                    ...
                    DriveSpan,
                    new(DirectorySpan.Start, index - DirectorySpan.Start)
                );
            }
        }
    ...
    public IReadOnlyCollection<string> GetComponents() {
        ...
        if (DirectorySpan.IsValid && DirectorySpan.Length >= 1) {
            list.AddRange(
                FullPath.Substring(DirectorySpan.Start, DirectorySpan.Length)
    ...
    public override bool Equals(object? obj) {
        ...
        return obj.GetType() == GetType() && Equals(obj as UPath);
    }
    ...
    public string ToWindowsPath() => FullPath.Replace('/', '\\');
    ...
    public static bool operator ==(UPath? left, UPath? right) => Equals(left, right);
    ...
    public static T Combine<T>(UDirectory? leftPath, T? rightPath) where T : UPath {
        if (leftPath == null) {
            throw new ArgumentNullException(nameof(leftPath));
        }

        if (rightPath == null) {
            throw new ArgumentNullException(nameof(rightPath));
        }
        ...
        var separator = string.IsNullOrEmpty(leftPath.FullPath) || string.IsNullOrEmpty(rightPath.FullPath)
            ? string.Empty
    ...
    public UPath MakeRelative(UDirectory anchorDirectory) {
        if (anchorDirectory == null) {
            throw new ArgumentNullException(nameof(anchorDirectory));
        }
        ...
        for (; index >= 0; index--) {
            ...
            if (index == absolutePath.Length
                || (index < absolutePath.Length && absolutePath[index] == DirectorySeparatorChar)) {
        ...
        relativePath.Append(absolutePath[index..]);
    ...
    public static implicit operator string?(UPath? url) => url?.FullPath;

    /// <summary>
    ///     Determines whether the specified path contains some directory characters '\' or '/'
    /// </summary>
    /// <param name="path">The path.</param>
    /// <returns><c>true</c> if the specified path contains some directory characters '\' or '/'; otherwise, <c>false</c>.</returns>
    public static bool HasDirectoryChars(string? path) =>
        path != null && (path.Contains(DirectorySeparatorChar) || path.Contains(DirectorySeparatorCharAlt));
    ...
    /// <summary>
    ///     Normalize a path by replacing '\' by '/' and transforming relative '..' or current path '.' to an absolute path.
    ///     See remarks.
    /// </summary>
    ...
    public static StringBuilder? Normalize(string pathToNormalize, out string? error) =>
        Normalize(pathToNormalize, out _, out _, out _, out error);
    ...
    /// <returns>A normalized path or null if there is an error.</returns>
    /// <remarks>Unlike <see cref="System.IO.Path" /> , this doesn't make a path absolute to the actual file system.</remarks>
    public static unsafe StringBuilder? Normalize(
        string? pathToNormalize,
        out StringSpan drive,
        out StringSpan directoryOrFileName,
        out StringSpan fileName,
        out string? error
    ) {
        drive = new();
        directoryOrFileName = new();
        fileName = new();
        error = null;
        ...
        var countDirectories = pathToNormalize.Count(
            pathItem => pathItem is DirectorySeparatorChar or DirectorySeparatorCharAlt or VolumeSeparatorChar
        );
        ...
        foreach (var pathItem in pathToNormalize) {
            ...
            if (pathItem is DirectorySeparatorChar or DirectorySeparatorCharAlt) {
                ...
                if (state != NormalizationState.DirectorySeparator) {
                    ...
                    if (state == NormalizationState.StartComponent || state == NormalizationState.VolumeSeparator) {
                        currentPath++;
                        paths[currentPath] = new(builder.Length, 1);
                    } else {
            ...
            } else if (pathItem == VolumeSeparatorChar) {
                ...
                if (state is NormalizationState.StartComponent or NormalizationState.DirectorySeparator) {
                    ...
                    currentPath++;
                    paths[currentPath] = new(builder.Length, 0);
                }
                ...
                error = $"Invalid character [{pathItem}] found in path [{pathToNormalize}]";
                return null;
            }
        }
        ...
        // Go back to upper path if current is not valid
    ...
    static int ComputeStringHashCodeCaseInsensitive(string text) {
    ...
    static bool IsParentComponentPath(StringBuilder builder, StringSpan path) {
        if (path.Length is 2 or 3 && builder[path.Start] == '.' && builder[path.Start + 1] == '.') {
    ...
    /// <param name="path">Span of component to compare against.</param>
    /// <returns>True if it represents a parent directory.</returns>
    static bool IsRelativeCurrentComponentPath(StringBuilder builder, StringSpan path) {
        if (path.Length is 1 or 2 && builder[path.Start] == '.') {
    ...
    /// <summary>
    ///     Trims the path by removing unnecessary '..' and '.' path items.
    /// </summary>
    ...
    static unsafe void TrimParentAndSelfPath(
    ...
    static string Decode(
        ...
        out StringSpan drive,
        out StringSpan directory,
        out StringSpan fileName,
        out StringSpan fileExtension
    ) {
        drive = new();
        directory = new();
        fileName = new();
