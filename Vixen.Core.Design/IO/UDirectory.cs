...
public sealed class UDirectory : UPath {
    ...
    internal UDirectory(string fullPath, StringSpan driveSpan, StringSpan directorySpan)
        : base(fullPath, driveSpan, directorySpan) { }
    ...
    public string GetDirectoryName() {
        var index = FullPath.LastIndexOf(DirectorySeparatorChar);
    ...
    public new UDirectory MakeRelative(UDirectory anchorDirectory) => (UDirectory)base.MakeRelative(anchorDirectory);
    ...
    public static implicit operator UDirectory?(string? fullPath) => fullPath != null ? new UDirectory(fullPath) : null;
    ...
    public static UDirectory Combine(UDirectory leftPath, UDirectory rightPath) => UPath.Combine(leftPath, rightPath);
    ...
    public bool Contains(UPath path) =>
        path.FullPath.StartsWith(FullPath, StringComparison.OrdinalIgnoreCase)
        && path.FullPath.Length > FullPath.Length
        && path.FullPath[FullPath.Length] == DirectorySeparatorChar;
}
