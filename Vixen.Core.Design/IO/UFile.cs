...
public sealed class UFile : UPath {
    ...
    public string? GetFileNameWithoutExtension() => NameSpan.IsValid ? FullPath.Substring(NameSpan) : null;

    /// <summary>
    ///     Gets the file path (<see cref="UPath.GetDirectory()" /> + '/' + <see cref="UFile.GetFileName()" />) with the
    ///     extension or drive. Can be an null if no filepath.
    /// </summary>
    ...
    /// <summary>
    ///     Gets the file path (<see cref="UPath.GetDirectory()" /> + '/' + <see cref="UFile.GetFileName()" />) without the
    ///     extension or drive. Can be an null if no filepath.
    /// </summary>
    /// <returns>The path.</returns>
    ...
    public string? GetFileExtension() => ExtensionSpan.IsValid ? FullPath.Substring(ExtensionSpan) : null;
    ...
    public static UFile Combine(UDirectory leftPath, UFile rightPath) => UPath.Combine(leftPath, rightPath);
    ...
    public new UFile MakeRelative(UDirectory anchorDirectory) => (UFile)base.MakeRelative(anchorDirectory);
    ...
    public new static bool IsValid(string path) {
        ...
        return !path.EndsWith(DirectorySeparatorChar, DirectorySeparatorCharAlt);
    }
    ...
    public static implicit operator UFile?(string? fullPath) => fullPath != null ? new UFile(fullPath) : null;
}
