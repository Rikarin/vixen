...
public class SetDescriptor : CollectionDescriptor {
    static readonly List<string> ListOfMembersToRemove = ["Comparer", "Capacity"];

    readonly MethodInfo addMethod;
    readonly MethodInfo removeMethod;
    readonly MethodInfo clearMethod;
    readonly MethodInfo containsMethod;
    readonly MethodInfo countMethod;
    readonly MethodInfo isReadOnlyMethod;
    ...
    public override bool IsReadOnly(object thisObject) =>
        thisObject == null || isReadOnlyMethod == null || (bool)isReadOnlyMethod.Invoke(thisObject, null);
    ...
    public override void Add(object set, object item) {
        ...
        if (set is ISet<object> simpleSet) {
            simpleSet.Add(item);
        } else {
            // Only throw an exception if the addMethod is not accessible when adding to a set
            if (addMethod == null) {
                throw new InvalidOperationException($"No Add() method found on set [{Type}]");
            }
    ...
    public override void Remove(object set, object key) {
        ...
        if (set is ISet<object> simpleSet) {
            ...
            if (removeMethod == null) {
                throw new InvalidOperationException($"No Remove() method found on set [{Type}]");
            }
    ...
    public bool Contains(object set, object value) {
        ...
        if (containsMethod == null) {
            throw new InvalidOperationException($"No Contains() method found on set [{Type}]");
        }

        return (bool)containsMethod.Invoke(set, [value]);
    }
    ...
    public override int GetCollectionCount(object set) =>
        set == null || countMethod == null ? -1 : (int)countMethod.Invoke(set, null);
    ...
    /// <returns></returns>
    public override object GetValue(object set, object index) => Contains(set, index) ? index : null;
    ...
    public override object GetValue(object set, int index) =>
        throw new InvalidOperationException(
            "SetDescriptor should not call function 'GetValue' with int index parameter."
        );
    ...
    public override void SetValue(object set, object index, object value) {
        ...
        if (set is ISet<object> simpleSet) {
            ...
            object[] indexParam = { index };
            object[] valueParam = { value };
